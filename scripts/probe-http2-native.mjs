...
const build = JSON.parse(readFileSync('public/http2-probe/build.json', 'utf8'))
...
const directory = mkdtempSync(join(tmpdir(), 'http2-native-'))
const args = ['-O1', '-g', '-fsanitize=address', '-fno-omit-frame-pointer', ...build.common, 'fixtures/http2-native.c', ...build.files, '-o', join(directory, 'probe')]
...
const report = { build, directory, args, hashes: Object.fromEntries(['fixtures/http2-native.c', 'scripts/probe-http2-native.mjs'].map(file => [file, hash(file)])), compile: summary(compile), execution: summary(execution) }
writeFileSync('reports/http2-native.json', JSON.stringify(report, null, 2) + '\n')
