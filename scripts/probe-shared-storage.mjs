...
import {runSharedStorage} from './shared-storage-harness.mjs'
...
const native=await runSharedStorage(engine)
...
if(process.argv.includes('--browser')){
  ...
  const server=createServer((req,res)=>{
    if(req.url==='/'){
      ...
      res.end(`<script type="module">import * as core from '/core.mjs';import factory from '/engine.mjs';import {QuickJSAsyncFFI} from '/ffi.mjs';import {runSharedStorage} from '/harness.mjs';try{const bytes=await(await fetch('/engine.wasm')).arrayBuffer();const engine=await core.newQuickJSAsyncWASMModuleFromVariant({type:'async',importFFI:async()=>QuickJSAsyncFFI,importModuleLoader:async()=>()=>factory({wasmBinary:bytes})});window.result=await runSharedStorage(engine)}catch(error){window.failure=String(error)}</script>`)
      ...
      res.end(readFileSync(req.url==='/harness.mjs'?'scripts/shared-storage-harness.mjs':join(root,req.url.slice(1))))
  ...
  try{
    for(const [name,type] of Object.entries({chromium,firefox,webkit})){
      ...
      try{
        ...
        console.log(name+': shared-storage ownership passed')
      }finally{await browser.close()}
    }
  }finally{
    writeFileSync('reports/shared-storage.json',JSON.stringify({scope:'Fixed-length QuickJS SAB ownership and atomic RMW, not worker transport, waits or shared WASM.',native,results},null,2)+'\n')
