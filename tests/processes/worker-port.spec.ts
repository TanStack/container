...
const source=readFileSync('tests/fixtures/worker-port-parent.mjs','utf8')
for(const guestWasm of [false,true])test(`queued MessagePort transfer ${guestWasm?'WASM':'default'}`,async({page})=>{
 const native=spawnSync(process.execPath,['tests/fixtures/worker-port-parent.mjs'],{encoding:'utf8',timeout:15000})
