...
for(const mode of ['sequential','concurrent'])test('Rolldown callable then sync TS composition '+mode,async({page},info)=>{
  test.setTimeout(60000)
  const source=`
const evidence={mode:${JSON.stringify(mode)},stages:[],results:[]};
try{
 const {transformSync}=await import('rolldown/utils');const {oxcRuntimePlugin}=await import('rolldown/experimental');const plugin=oxcRuntimePlugin(),handler=typeof plugin.transform==='function'?plugin.transform:plugin.transform.handler;evidence.stages.push('imported');
 const run=async i=>{evidence.stages.push('start-'+i);const filename='/project/input-'+i+'.ts',code='export const value: number = '+(40+i)+';';const first=await handler.call({},code,filename,{ssr:false,moduleType:'ts'});const r=transformSync(filename,first?.code??code,{lang:'ts',sourcemap:true});evidence.stages.push('done-'+i);return {code:r.code,errors:r.errors}};
