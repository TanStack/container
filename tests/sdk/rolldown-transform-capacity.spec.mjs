...
const policy={experimentalFibers:true,maxBytes:128*1024*1024,workerMaxBytes:Number(process.env.ROLLDOWN_WORKER_MIB??128)*1024*1024,timeoutMs:15000,sharedMemoryPerEngine:{maxBytes:1280*1024*1024,growthReservation:true},workspace:{maxBytes:64*1024*1024}}
...
for(const mode of ['sequential','concurrent'])test('standalone Rolldown three TS transforms '+mode,async({page},info)=>{
  test.setTimeout(60000)
  const source=`
const evidence={mode:${JSON.stringify(mode)},stages:[],results:[]};
try{
 const {transform}=await import('rolldown/utils');evidence.stages.push('imported');
 const run=async i=>{evidence.stages.push('start-'+i);const r=await transform('/project/input-'+i+'.ts','export const value: number = '+(40+i)+';',{lang:'ts'});evidence.stages.push('done-'+i);return {code:r.code,errors:r.errors}};
