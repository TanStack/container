...
import {prepareRuntimeAssets,readPreviewHostHostingContract} from '@tanstack/browser-sandbox-experimental/assets'
...
const workspace=mkdtempSync(join(tmpdir(),'sdk-basic-host-'))
...
export async function startExample(options={}){
  const {ownerPort,previewPort}=examplePorts({ownerPort:4173,previewPort:4174},options)
  ...
  const owner=createServer((request,response)=>{
    try{
      ...
      if(path==='/config.json'){response.setHeader('Content-Type','application/json');response.end(JSON.stringify({previewOrigin}));return}
      ...
      if(path==='/'||path==='/client.js')return sendFile(response,here,path==='/'?'index.html':'client.js')
